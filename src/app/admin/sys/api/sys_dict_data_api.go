...
func (api *SysDictDataApi) Delete(c *gin.Context) {
	...
	if err := api.sysDictDataService.DeleteByIds(ids); err != nil {
...
func (api *SysDictDataApi) Get(c *gin.Context) {
	id := c.Param("id")
	if err, sysDictDataView := api.sysDictDataService.Get(id); err != nil {
...
func (api *SysDictDataApi) GetByType(c *gin.Context) {
	...
	if err, sysDictDataView := api.sysDictDataService.GetByType(dictType); err != nil {
		global.Logger.Error("查询失败!", zap.Error(err))
		response.FailWithMessage("查询失败", c)
	} else {
		response.OkWithData(sysDictDataView, c)
...
func (api *SysDictDataApi) Page(c *gin.Context) {
	...
	if err, res := api.sysDictDataService.Page(&pageInfo); err != nil {
